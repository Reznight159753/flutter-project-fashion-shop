...
$query = "SELECT sp.MA_SAN_PHAM AS id, sp.TEN_SAN_PHAM AS name, sp.GIA_BAN AS price, sp.SO_LUONG_TON AS stock,
    sp.MO_TA AS description, sp.MA_DANH_MUC AS category_id, sp.HINH_ANH AS image,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        if (empty($_POST['name']) || !is_numeric($_POST['price']) || !is_numeric($_POST['stock'])) {
            echo "<script>alert('Invalid input for adding product.');</script>";
        } else {
            $imageName = null;
            if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
                $targetDir = __DIR__ . '/../shop_api/images/';
            ...
            $stmt->execute([$_POST['name'], $_POST['price'], $_POST['stock'], $_POST['category'], $_POST['description'], $imageName]);
            header("Location: index.php");
            exit;
        }
    } elseif ($action === 'update') {
        if (empty($_POST['name']) || !is_numeric($_POST['price']) || !is_numeric($_POST['stock']) || empty($_POST['id'])) {
            echo "<script>alert('Invalid input for updating product.');</script>";
        } else {
            $imageName = null;
            if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
                $targetDir = __DIR__ . '/../shop_api/images/';
            ...
            if ($imageName !== null) {
                ...
                $stmt->execute([$_POST['name'], $_POST['price'], $_POST['stock'], $_POST['category'], $_POST['description'], $imageName, $_POST['id']]);
            } else {
                $stmt = $pdo->prepare("UPDATE SAN_PHAM SET TEN_SAN_PHAM=?, GIA_BAN=?, SO_LUONG_TON=?, MA_DANH_MUC=?, MO_TA=? WHERE MA_SAN_PHAM=?");
                $stmt->execute([$_POST['name'], $_POST['price'], $_POST['stock'], $_POST['category'], $_POST['description'], $_POST['id']]);
            }
            header("Location: index.php");
            exit;
        }
                    ...
                    <tbody>
                        ...
                        <tr>
                            <form method="post" enctype="multipart/form-data">
                                ...
                                <td>
                                    <input type="number" name="price" step="1000" value="<?= $p['price'] ?>" style="width: 120px;">
                                    <?= number_format($p['price'], 0, ',', '.') ?> VND
                                </td>
                                <td><input type="number" name="stock" value="<?= $p['stock'] ?>" style="width: 80px;"></td>
                                <td><?= $p['sold'] ?></td>
                                <td>
                                ...
                                <td>
                                    <textarea name="description" onclick="this.rows=5;" onblur="this.rows=1;" rows="1"><?= htmlspecialchars($p['description']) ?></textarea>
                                </td>
                                <td>
                                    <input type="file" name="image" accept="image/*">
                                    <?php if (!empty($p['image'])): ?>
                                        <img src="../shop_api/images/<?= htmlspecialchars($p['image']) ?>" alt="Product Image" style="width: 50px; height: 50px;">
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <button class="btn btn-primary" type="submit">Apply Edit</button>
                                    <a class="btn btn-primary" href="?remove_id=<?= $p['id'] ?>" onclick="return confirm('Remove this product?')">Remove</a>
                                </td>
                            </form>
                        </tr>
                        <?php endforeach; ?>
